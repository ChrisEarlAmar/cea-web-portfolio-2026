...
export default function Experience() {
  return (
    <section id="experience" className="py-24 bg-[#111915]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          {experience.map((job) => (
            <div
              key={job.id}
              className="p-8 bg-[#0a0f0d] rounded-2xl border border-[#1F6F5F]/20 hover:border-[#2FA084]/40 transition-all duration-300 group"
            >
              {/* Header Row */}
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-6">
                <div className="flex items-start gap-4">
                  <div className="w-14 h-14 bg-[#1F6F5F]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                    <Building2 className="w-7 h-7 text-[#2FA084]" />
                  </div>
                  <div>
              ...
              {/* Description */}
              <p className="text-[#9ca3af] mb-6 leading-relaxed max-w-4xl">
                {job.description}
              </p>
                  ...
                  <div className="space-y-3">
                    {job.highlights.map((highlight, hIndex) => (
                      <div key={hIndex} className="flex items-start gap-3">
                        ...
                        <span className="text-[#9ca3af] text-sm">{highlight}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {job.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-3 py-1.5 bg-[#16241f] text-[#9ca3af] text-sm rounded-lg border border-[#1F6F5F]/20 hover:border-[#2FA084]/40 transition-colors"
                      >
                        {tech}
                      </span>
                    ))}
